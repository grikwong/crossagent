...
// Only allowed when the workflow is at phase 1, has no plan artifact yet,
// and has no active plan session running (i.e., the workflow has not started).
func handleWorkflowSetDescription(sm *SessionManager) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Enforce server-side: only allow editing if phase is "1", plan.md is absent,
		// and no plan session is currently running.
		...
		if sm != nil && sm.GetByWorkflowPhase(name, "plan") != nil {
			writeError(w, 409, "description can only be updated before the workflow is run")
			return
		}
